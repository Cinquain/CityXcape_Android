...
data class User(
    ...
    companion object {
        ...
        fun CreateFromMap(data: Map<String, Any?>): User {
            val worldsMapRaw = data["worlds"] as? Map<*, *> ?: emptyMap<Any, Any>()

            val fetchedWorlds = worldsMapRaw.mapNotNull { (_, value) ->
                val worldData = value as? Map<*, *> ?: return@mapNotNull null

                @Suppress("UNCHECKED_CAST")
                val worldMap = worldData as Map<String, Any?>

                World.CreateFromMap(worldMap)
            }
            ...
            return User(
                ...
                streetcred = (data["streetcred"] as? Long)?.toInt() ?: 0,
